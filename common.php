...
function main($path) {
    ...
    if ($_SERVER['ajax']) {
        ...
        if ($_GET['action'] == 'del_upload_cache') {
            // del '.tmp' without login. 無需登入即可刪除.tmp後綴文件
...
function findIndexPath($rootpath, $path = '') { // find the path of the first 'index.php' that not in rootpath.
    ...
    $handler = opendir(path_format($rootpath . $slash . $path)); //打開當前文件夾
    while ($filename = readdir($handler)) {
        if ($filename != "." && $filename != "..") { //文件夾文件名字為'.'和'..'，不要對它們進行操作
...
function output($body, $statusCode = 200, $headers = ['Content-Type' => 'text/html'], $isBase64Encoded = false) {
    ...
    if (baseclassofdrive() == 'BaiduDisk') $headers['Referrer-Policy'] = 'no-referrer';
...
<script>
    function changedrivetype(d) {
        ...
        $frame .= '
        <a href="https://github.com/SAOJSM/OneManager-php" target="_blank">Github</a>
        <a href="https://gitee.com/SAOJSM/OneManager-php" target="_blank">Gitee</a>
...
<form name="updateform" action="" method="post">
    ...
    <input type="text" name="auth" size="6" placeholder="auth" value="SAOJSM">
...
<script>
    function changeGitSource(d) {
        if (d.options[d.options.selectedIndex].value=="Github") document.updateform.auth.value = "SAOJSM";
        if (d.options[d.options.selectedIndex].value=="Gitee") document.updateform.auth.value = "SAOJSM";
